...
def trace_for_chunk(xt_, rank, chunkSize, num_gpus,currentInv,fisher, gpu_id ):
    ...
    # print(currentInv.shape, fisher.shape, xt_chunk.shape)
    innerInv1 = torch.eye(rank).cuda(gpu_id) 
    # print('here')
    innerInv = torch.inverse(innerInv1 + xt_chunk @ currentInv @ xt_chunk.transpose(1, 2)) 
...
def select(X, K, fisher, iterates, lamb=1, backwardSteps=0, nLabeled=0, chunkSize=200):
    ...
    start = time.time()
    fisher = [fisher.clone().detach().cuda(0), fisher.clone().detach().cuda(1), fisher.clone().detach().cuda(2), fisher.clone().detach().cuda(3)]
    xts = [xt_.clone().detach().cuda(0), xt_.clone().detach().cuda(1), xt_.clone().detach().cuda(2),  xt_.clone().detach().cuda(3)]
    cInvs = [currentInv.clone().detach().cuda(0), currentInv.clone().detach().cuda(1), currentInv.clone().detach().cuda(2),currentInv.clone().detach().cuda(3)]
    mid = time.time()
    print('Time for sending to GPU', mid - start)
